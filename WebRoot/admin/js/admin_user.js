...
function initDataTable(){
	$("#dg").datagrid({
		...
		loadMsg:"正在加载数据，请稍候……",
		columns:[[
		    {field:'op',title:'操作',width:'80',
		    	formatter: function(value,row,index){
		    		return '<input type="button" value="设置定位点" onclick="showMapWin(\''+row.id+'\',\''+row.longitude+'\',\''+row.latitude+'\')"/>';
		    	}
		    },     
			{field:'id',title:'编号',width:'40',hidden:true},
			{field:'rolename',title:'用户角色',width:'80'},
			{field:'username',title:'用户名',width:'80'},
			{field:'userpwd',title:'用户密码',width:'70'},
			{field:'name',title:'姓名',width:'70'},
			{field:'department',title:'所属部门',width:'110',sortable:true},
			{field:'duty',title:'职务名称',width:'70',sortable:true},
...
function initPersonList(){
	...
	for(var i=0;i<count;i++){
		...
		if(!isUser){
			text = rcd.name+"　[所属部门："+rcd.department+"，联系电话："+rcd.telephone+"]";
...
function getUserName(){
	var name = $("#personid").find("option:selected").attr("name");
	if(name!=null){
		Ajax.getNamePinYin(name,function(data){
			$("#username").val(data);
		});
	}
}
...
function doSubmitAction(){
	...
	top.layerIndex = top.showLayerLoading("正在保存数据，请稍候……");
