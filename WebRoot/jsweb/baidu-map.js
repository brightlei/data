...
function showLayerData(data,type){
	clearLayerTypeMarkers(type);
	if(type=="t_shortmsg"){
		showShopDailyPlan(data,type);
		flashShortMsgLayer();
	}else{
		showBusinessLayer(data,type);
	}
}
/**
 * 显示车间日计划数据
 * @param data 业务图层数据[{},{}]
 */
function showShopDailyPlan(data,type){
 ...
 * @param data 业务图层数据[{},{}]
 */
function showBusinessLayer(data,type){
	var count = data.length;
	var t = null;
	for(var i=0;i<count;i++){
		t = data[i];
		if(t.longitude!=""&&t.latitude!=""){
			var mk = addLayerMarker(t,type);
			mapOverlayers[type].push(mk);
		}
	}
}
 ...
 * @param type 图层类型
 */
function drawJobLine(record,type){
	//里程坐标点信息
	var mileagePoints = getLonlatByMileage(record.kslcs,record.jslcs);
	//开始里程坐标点
	var startPoint = mileagePoints[0];
	//结束里程坐标点
	var endPoint = mileagePoints[1];
	//要显示的字段信息
	...
	//添加线路起点标注点
	addJobLineMarker(startPoint,"start",infoWindow,type);
	//添加线路终点标注点
	addJobLineMarker(endPoint,"end",infoWindow,type);
	//绘制线路
	var pntArr = [];
	var point = null;
	var count = mileagePoints.length;
	for(var i=0;i<count;i++){
		point = mileagePoints[i];
		pntArr.push(new BMap.Point(parseFloat(point.longitude), parseFloat(point.latitude)));
	}
	var line = new BMap.Polyline(pntArr, {strokeColor:"red", strokeWeight:6, strokeOpacity:0.8});   //创建折线
	line.laytype = "line";
  	map.addOverlay(line);
  	mapOverlayers[type].push(line);
}
/**
 * 添加任务线路标注点
 * @param point 标注点
 * @param pointType 标注点类型:start-开始点,end-结束点
 * @param infoWindow 标注点气泡内容
 * @param layerType 图层类型
 */
function addJobLineMarker(pnt,pointType,infoWindow,layerType){
	var lon = parseFloat(pnt.longitude);
	var lat = parseFloat(pnt.latitude);
	var point = new BMap.Point(lon, lat);
  	var myIcon = new BMap.Icon("images/icon-"+pointType+".png", new BMap.Size(parseInt(36,10),parseInt(36,10)));
	marker = new BMap.Marker(point,{icon:myIcon});
	marker.addEventListener("click", function(){
  		this.openInfoWindow(infoWindow);
 	});
	marker.info = {layertype:layerType,lon:lon,lat:lat,name:"aaaa"};
	marker.laytype = "marker";
	map.addOverlay(marker);
	mapOverlayers[layerType].push(marker);
}
...
function addLayerMarker(info,type){
	...
	marker.info = {layertype:type,lon:lon,lat:lat,name:info[viewColumn]};
	marker.laytype = "marker";
	marker.addEventListener("click", function(){          
	   this.openInfoWindow(infoWindow);
	});
...
//车间日计划数据在地图上闪烁显示
function flashShortMsgLayer(){
	flashTimeout = setTimeout(function(){
		var data = mapOverlayers["t_shortmsg"];
		if(flashState=="show"){
			for(var i=0;i<data.length;i++){
				if(data[i].laytype=="line"){
					data[i].hide();
				}
			}
			flashState = "hide";
...
//清除闪烁
function clearFlashShortMsgLayer(){
...
//清除指定的标注点
function clearLayerTypeMarkers(type){
	var data = mapOverlayers[type];
	...
	//清除车间日计划闪烁定时任务
	clearFlashShortMsgLayer();
	console.log(mapOverlayers[type],type);
}
//根据里程获取坐标点信息
...
//添加点击标注点
function addClickPointMarker(){
	var lon = top.queryCirclePoint.lon;
	var lat = top.queryCirclePoint.lat;
	var point = new BMap.Point(lon, lat);
